...
{	
    ...
    //cv::subtract(reference_image, m_depthImage, m_depthImage);
    
    // thresholding
    cv::Mat m_depthImage8CU1;
    m_depthImage.convertTo(m_depthImage8CU1, CV_8UC1);
    cv::threshold(m_depthImage8CU1, m_depthImage8CU1, 250, 0, cv::THRESH_TOZERO);
    m_depthImage8CU1.convertTo(m_depthImage, CV_16UC1);
...
void Application::loop()
{
    ...
    if (!set_reference_image) {
        reference_image = m_depthImage.clone() * 15;
        set_reference_image = true;
    }
